  ...
  <xsl:text>namespace </xsl:text><xsl:value-of select="$platform/parameters/parameter[@name='entity-namespace']"/><xsl:text> 
{
  ...
  /// &lt;see cref="</xsl:text><xsl:value-of select="$platform/parameters/parameter[@name='entity-namespace']"/><xsl:text>.</xsl:text><xsl:value-of select="$entity"/><xsl:text>"/&gt;
    ...
    <xsl:text>  /// &lt;see cref="PVDbObjectHome&amp;lt;T&amp;gt;"/&gt;
  ...
  <!-- Class Beginn -->
  <xsl:text>  public partial class </xsl:text><xsl:value-of select="$entity"/><xsl:text>Home : PVDbObjectHome&lt;</xsl:text><xsl:value-of select="$entity"/><xsl:text>&gt;
  {
  ...
  <!-- foreign key dependent find methods. No longer needed due to generic find method. -->
...
<xsl:text>
    ...
    {
      var dbo = new </xsl:text><xsl:value-of select="$entity"/>
      <xsl:text>
      {</xsl:text>
    <xsl:for-each select="attributes/attribute[position()!=1 and @required='true' and (@default-value='' or not(@default-value)) and @type!='BIT' and @type!='VSTAMP']">
        <xsl:text>
        </xsl:text><xsl:value-of select="@name"/><xsl:text> = p</xsl:text>
        <xsl:call-template name="type-prefix">
          <xsl:with-param name="type" select="@type"/>
        </xsl:call-template>
        <xsl:value-of select="@name"/>
...
<xsl:text>
    ...
    <xsl:text>PVDbConnection pcon)
    {
      var dbo = new </xsl:text><xsl:value-of select="$entity"/><xsl:text>
      {</xsl:text>
    <xsl:for-each select="method-attribute">
        <xsl:text>
        </xsl:text>
...
<xsl:text>
    ...
    /// &lt;exception cref="PVExceptionNoRecordFound"&gt;Is thrown when no entity with the given primary key is found.&lt;/exception&gt;
</xsl:text>
    </xsl:if>
    <xsl:text>    public static new </xsl:text><xsl:value-of select="$entity"/><xsl:text> FindByPrimaryKey(int pID, PVDbConnection pcon)
      => PVDbObjectHome&lt;</xsl:text><xsl:value-of select="@name"/><xsl:text>&gt;.FindByPrimaryKey(pID, pcon);
</xsl:text>
  </xsl:if>
...
<xsl:text>
    ...
    <xsl:text>    public static List&lt;</xsl:text><xsl:value-of select="$entity"/><xsl:text>&gt; </xsl:text>
    ...
    <xsl:text>, PVDbConnection pcon)
    {
      var stmt = GetDbPreparedStatementMultiSelect(
    ...
    <xsl:text>        var result = ExecuteStatementToArray(stmt);
        return result;
</xsl:text>
    <xsl:text>      }
    }
...
<xsl:text>
    ...
    <xsl:if test="@returns-multiple='true'"><xsl:text>List&lt;</xsl:text></xsl:if>
    ...
    <xsl:if test="inner-join">
      ...
      <xsl:text>        "</xsl:text><xsl:value-of select="@entity-alias"/><xsl:text>", // Table alias
        new[] { </xsl:text><xsl:value-of select="$tables"/><xsl:text> }, // Inner join table(s)
        new[] { </xsl:text><xsl:value-of select="$conditions"/><xsl:text> }, // Inner join condition(s)
</xsl:text>
    </xsl:if>
    <xsl:if test="@returns-multiple='false'">
