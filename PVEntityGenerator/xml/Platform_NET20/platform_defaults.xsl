...
<xsl:variable name="platform" select="$project-settings/platforms/platform[@name='NET20']"/>

<xsl:variable name="generate-comments" select="$platform/parameters/parameter[@name='generate-comments']='true'"/>

<xsl:variable name="entity-class" select="concat($platform/parameters/parameter[@name='entity-namespace'],'.',$entity)"/>
...
<xsl:key name="generate-entity-platform" match="/pventitygenerator-data/project-settings/platforms/platform[@name='NET20']/entity-generation/generate-entity" use="@entity"/>
...
<!-- Formatiert einen Wert für NET-Code abhängig vom Typ -->
<xsl:template name="format-type-value">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$type='TIMESTAMP'">
      <xsl:text>PVFramework.Util.PVFormatUtil.ParseDateTime(&quot;</xsl:text>
      <xsl:value-of select="$value"/>
      <xsl:text>&quot;)</xsl:text>
    </xsl:when>
...
<!-- Gibt einen NET-Type zum Datentyp zurück -->
...
<!-- Gibt einen Enumerations-Type zu einer attribute zurück -->
...
<!-- Gibt eine NET-Type-Bezeichnung für get/set-Methodenaufrufe zum Datentyp zurück -->
...
<xsl:template name="test-test-value">
  ...
  <xsl:if test="$enum-type!='' and $check-enum=1">
    <xsl:text>(</xsl:text>
  ...
  <xsl:choose>
    <xsl:when test="$value-type='VARCHAR'">
      <xsl:variable name="chars100" select="'üüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüüü'"/>
      <xsl:variable name="chars1000" select="concat($chars100,$chars100,$chars100,$chars100,$chars100,$chars100,$chars100,$chars100,$chars100,$chars100)"/>
      <xsl:variable name="chars10000" select="concat($chars1000,$chars1000,$chars1000,$chars1000,$chars1000,$chars1000,$chars1000,$chars1000,$chars1000,$chars1000)"/>
      <xsl:value-of select="concat('&quot;',substring($chars10000,1,$attribute/@size),'&quot;')"/>
    </xsl:when>
    ...
    <xsl:when test="$value-type='TIMESTAMP'">
      <xsl:text>PVFramework.Util.PVFormatUtil.ParseDateTime("15.03.2001 16:20:15")</xsl:text>
    </xsl:when>
...
<xsl:template name="test-default-value">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$attribute/@type='TIMESTAMP'">
      ...
      <xsl:choose>
        <xsl:when test="$value!=''">
          <xsl:text>PVFramework.Util.PVFormatUtil.ParseDateTime("</xsl:text><xsl:value-of select="$value"/><xsl:text>")</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>null</xsl:text>
        </xsl:otherwise>
