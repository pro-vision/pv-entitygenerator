...
<xsl:if test="$generate-entity/parameters/parameter[@name='generate-entity']='true'
  ...
  <xsl:text>namespace </xsl:text><xsl:value-of select="$platform/parameters/parameter[@name='entity-namespace']"/><xsl:text>
{
  using System;
  ...
  /// &lt;see cref="</xsl:text><xsl:value-of select="$entity"/><xsl:text>Home"/&gt;
</xsl:text>
    </xsl:if>
    <xsl:text>  /// &lt;see cref="PVDbObject"/&gt;
  ...
  <xsl:choose>
    <xsl:when test="$generate-entity/parameters/parameter[@name='generate-entity']='true'">
      <xsl:text>  public partial class </xsl:text><xsl:value-of select="$entity"/><xsl:text> : PVDbObject
  {
</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>  public abstract partial class </xsl:text><xsl:value-of select="$entity"/><xsl:text>
  {
</xsl:text>
    </xsl:otherwise>
  </xsl:choose>
...
<xsl:template match="entity" mode="constructor">
  <xsl:if test="$generate-comments">
    ...
    <xsl:text>      this.PerformCustomInitialization();
      this.InitializeAttributeValueChangedHandler();
    }
</xsl:text>
...
<xsl:template match="entity" mode="attributes">
  ...
  <xsl:text>
    /// &lt;summary&gt;
    /// Allows for custom initialization from other partial definitions of this class to run upon object creation.
    /// &lt;/summary&gt;
    partial void PerformCustomInitialization();
</xsl:text>
...
<xsl:text>
    ...
    <xsl:text>this.mattr[</xsl:text><xsl:value-of select="position()-1"/><xsl:text>].</xsl:text>
    ...
    <xsl:text>      set => this.mattr[</xsl:text><xsl:value-of select="position()-1"/><xsl:text>].</xsl:text>
    <xsl:call-template name="method-type">
      <xsl:with-param name="type" select="@type"/>
    </xsl:call-template>
...
<xsl:text>
    ...
    <xsl:for-each select="$foreign-key-check">
      ...
      <xsl:text>
      // Check Table </xsl:text><xsl:value-of select="$foreign-entity"/><xsl:text>.
...
<xsl:text>
    ...
    <xsl:for-each select="$foreign-key-cascading-delete">
      <xsl:text>
      // Remove from table </xsl:text><xsl:value-of select="../../@name"/><xsl:text>
      var list</xsl:text>
      <xsl:value-of select="../../@name"/><xsl:value-of select="position()"/><xsl:text> = </xsl:text>
      <xsl:text>StaticPVDbObjectHome&lt;</xsl:text><xsl:value-of select="../../@name"/><xsl:text>&gt;.FindBy(&quot;</xsl:text>
      <xsl:value-of select="attribute-ref/@attribute"/>
      <xsl:text>&quot;, this.</xsl:text><xsl:value-of select="attribute-ref/@foreign-attribute"/><xsl:text>, pcon);
      foreach (var dbo in list</xsl:text><xsl:value-of select="../../@name"/><xsl:value-of select="position()"/><xsl:text>)
      {
        dbo.Remove(pcon);
      }
</xsl:text>
...
<xsl:template match="entity" mode="enumeration-methods">
  ...
  <xsl:text>    public static System.Collections.ArrayList GetEnumNames() => new System.Collections.ArrayList(</xsl:text><xsl:value-of select="count($generate-entity/enumeration-entries/enumeration-entry[@generate='true'])"/><xsl:text>)
    {</xsl:text>
  <xsl:for-each select="$generate-entity/enumeration-entries/enumeration-entry[@generate='true']">
    <xsl:sort select="@identifier"/>
    <xsl:text>
      new PVFramework.Util.PVKeyValueItem(</xsl:text><xsl:value-of select="$entity"/><xsl:text>Enum.</xsl:text><xsl:value-of select="@identifier"/>
    <xsl:text>, "</xsl:text>
    <xsl:call-template name="replace-string">
      <xsl:with-param name="text" select="@name"/>
      <xsl:with-param name="replace" select="'&quot;'"/>
