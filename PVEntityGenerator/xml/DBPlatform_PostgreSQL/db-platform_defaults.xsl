...
<xsl:variable name="db-platform" select="$project-settings/db-platforms/db-platform[@name='PostgreSQL']"/>
...
<xsl:template match="entity" mode="drop">
  <xsl:text>SELECT drop_if_exists('</xsl:text><xsl:value-of select="@name"/><xsl:text>','DROP TABLE </xsl:text><xsl:value-of select="@name"/><xsl:text> CASCADE');
</xsl:text>
</xsl:template>
<!-- Drop table -->
<xsl:template match="export-entity|patch-entity" mode="drop">
  <xsl:text>DROP TABLE </xsl:text><xsl:value-of select="@entity"/><xsl:text> CASCADE;
</xsl:text>
</xsl:template>
