...
<xsl:template match="patch-entity" mode="modify">
  ...
  <xsl:if test="patch-attribute[@patch-type='CREATE']">
    ...
    <xsl:for-each select="patch-attribute[@patch-type='CREATE']">
      <xsl:if test="$db-definition/entities/entity[@name=$entity]/attributes/attribute[@name=current()/@name]">
        <xsl:if test="position() &gt; 1">
      <xsl:text>,
</xsl:text>
        </xsl:if>
        <xsl:apply-templates select="$db-definition/entities/entity[@name=$entity]/attributes/attribute[@name=current()/@name]" mode="create"/>
      </xsl:if>
    </xsl:for-each>
  <xsl:text>);
</xsl:text>
  ...
  <xsl:if test="patch-attribute[@patch-type='MODIFY']">
    ...
    <xsl:if test="$mod">
    <xsl:text>ALTER TABLE </xsl:text><xsl:value-of select="$entity"/><xsl:text> MODIFY COLUMN</xsl:text>
