 ...
 */
  ...
  <xsl:choose>
    <xsl:when test="$generate-entity/parameters/parameter[@name='generate-entity']='true'">
      <xsl:text>public class </xsl:text><xsl:value-of select="$entity"/><xsl:text> extends PVDbObject {

</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>public class </xsl:text><xsl:value-of select="$entity"/><xsl:text> {

  private </xsl:text><xsl:value-of select="$entity"/><xsl:text>() {
    // "Enumeration-Class" with static fields only; instancing or subclassing not allowed
  }
...
<xsl:template match="entity" mode="get-set">
   ...
   */
    ...
    */
    ...
    return mattr[</xsl:text><xsl:value-of select="position()-1"/><xsl:text>].get</xsl:text>
...
<xsl:template match="entity" mode="constraint-checks">
   ...
   */
    ...
    <xsl:text>  protected void checkRemoveConstraints(PVDbConnection pcon) throws PVException, PVExceptionRemoveConstraint {
   ...
   */
    ...
    <xsl:text>  protected void removeDependentObjects(PVDbConnection pcon) throws PVExceptionDbConcurrency, PVExceptionRemoveConstraint, PVException {
...
<xsl:template match="entity" mode="enumeration-methods">
  ...
  <!-- Mapping-Methode um den Namen zur Enumeration zurück geben zu können -->
  <xsl:if test="$generate-comments">
    <xsl:text>  /**
   * Gibt den Namen zur Konstante aus der Enumeration zurück.
   * @param pID Konstante aus </xsl:text><xsl:value-of select="$entity"/><xsl:text>.
   * @return Der passende Name zur Konstante.
   */
</xsl:text>
  </xsl:if>
  <xsl:text>  public static String getEnumName(int pID) {
    switch (pID) {
</xsl:text>
  <xsl:for-each select="$generate-entity/enumeration-entries/enumeration-entry[@generate='true']">
    <xsl:sort select="@identifier"/>
    <xsl:text>      case </xsl:text><xsl:value-of select="@identifier"/><xsl:text>:
        return "</xsl:text>
    <xsl:call-template name="replace-string">
