...
<xsl:text>  public void testCreateStoreRemoveAttributes() throws Exception {
  ...
  <xsl:for-each select="attributes/attribute[position()!=1 and @type!='VSTAMP' and not(@no-unittest='true') and not(@deprecated='true') and (@required='false' or @type!='BLOB')]">
    <xsl:text>    Assert.assertEquals("test default value </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
    <xsl:choose>
  ...
  <xsl:for-each select="attributes/attribute[position()!=1 and @type!='VSTAMP' and not(@no-unittest='true') and not(@deprecated='true')]">
    ...
    for (int intIndex=0; intIndex&lt;abytGet_</xsl:text><xsl:value-of select="position()"/><xsl:text>.length; intIndex++) {
      Assert.assertEquals("test get BinaryData", abytGet_</xsl:text><xsl:value-of select="position()"/><xsl:text>[intIndex], dbo.get</xsl:text><xsl:value-of select="@name"/><xsl:text>()[intIndex]);
    }
</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>    Assert.assertEquals("test get </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
        <xsl:call-template name="test-test-value">
          <xsl:with-param name="attribute" select="."/>
        </xsl:call-template>
        <xsl:text>, dbo.get</xsl:text><xsl:value-of select="@name"/><xsl:text>()</xsl:text>
  ...
  <xsl:for-each select="attributes/attribute[position()!=1 and @type!='VSTAMP' and not(@no-unittest='true') and not(@deprecated='true')]">
    ...
    for (int intIndex=0; intIndex&lt;abytRetrieve_</xsl:text><xsl:value-of select="position()"/><xsl:text>.length; intIndex++) {
      Assert.assertEquals("test retrieve BinaryData", abytRetrieve_</xsl:text><xsl:value-of select="position()"/><xsl:text>[intIndex], dbo2.get</xsl:text><xsl:value-of select="@name"/><xsl:text>()[intIndex]);
    }
</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>    Assert.assertEquals("test retrieve </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
        <xsl:call-template name="test-test-value">
          <xsl:with-param name="attribute" select="."/>
        </xsl:call-template>
...
<xsl:template match="entity" mode="test-constraints">
   ...
   */
    ...
    dbo2.remove(mcon);
    Assert.assertEquals("Constraint must be thrown", true, fRemoveConstraint);
    dbo1.remove(mcon);
  }
   ...
   */
    ...
    Assert.assertEquals("Record must be deleted", null, dbo3);
  }
   ...
   */
    ...
    <xsl:for-each select="method-attribute">
      ...
      <xsl:if test="$attribute/@type!='VSTAMP'">
        <xsl:text>    Assert.assertEquals("test constructor value </xsl:text><xsl:value-of select="@attribute-name"/><xsl:text>", </xsl:text>
        <xsl:call-template name="test-test-value">
          <xsl:with-param name="attribute" select="$attribute"/>
        </xsl:call-template>
...
<xsl:template match="custom-find-method" mode="test-standard">
   ...
   */
    ...
    for (int intIndex=0; intIndex&lt;adbo.length; intIndex++) {
      if (adbo[intIndex].get</xsl:text><xsl:value-of select="$ent/attributes/attribute[1]/@name"/><xsl:text>()==dbo.get</xsl:text>
        <xsl:value-of select="$ent/attributes/attribute[1]/@name"/><xsl:text>()) {
        fFound = true;
      }
    }
    Assert.assertEquals("test </xsl:text><xsl:value-of select="@name"/><xsl:text> method", true, fFound);
</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text> </xsl:text>
    ...
    Assert.assertEquals("test </xsl:text><xsl:value-of select="@name"/><xsl:text> method", true, dbo2!=null);
</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
