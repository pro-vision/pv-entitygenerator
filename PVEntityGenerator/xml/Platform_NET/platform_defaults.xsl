...
<xsl:variable name="platform" select="$project-settings/platforms/platform[@name='NET']"/>
...
<xsl:key name="generate-entity-platform" match="/pventitygenerator-data/project-settings/platforms/platform[@name='NET']/entity-generation/generate-entity" use="@entity"/>
...
<xsl:template name="format-type-value">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$type='TIMESTAMP'">
      <xsl:text>PVFormatUtil.ParseDateTime(&quot;</xsl:text>
      <xsl:value-of select="$value"/>
      <xsl:text>&quot;)</xsl:text>
    </xsl:when>
...
<xsl:template name="test-test-value">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$value-type='TIMESTAMP'">
      <xsl:text>PVFormatUtil.ParseDateTime("15.03.2001 16:20:15")</xsl:text>
    </xsl:when>
...
<xsl:template name="test-default-value">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$attribute/@type='TIMESTAMP'">
      ...
      <xsl:choose>
        <xsl:when test="$value!=''">
          <xsl:text>PVFormatUtil.ParseDateTime("</xsl:text><xsl:value-of select="$value"/><xsl:text>")</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>null</xsl:text>
        </xsl:otherwise>
