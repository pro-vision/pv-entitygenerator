  ...
  <xsl:for-each select="attributes/attribute[position()!=1 and @type!='VSTAMP' and not(@no-unittest='true') and not(@deprecated='true') and (@required='false' or @type!='BLOB')]">
    ...
    <xsl:choose>
      <xsl:when test="@type='TIMESTAMP' and $value='null'">
        <xsl:text>      Assertion.Assert("test default value </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
        <xsl:text>dbo.</xsl:text><xsl:value-of select="@name"/><xsl:text>.IsNull</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>      Assertion.AssertEquals("test default value </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
  ...
  <xsl:for-each select="attributes/attribute[position()!=1 and @type!='VSTAMP' and not(@no-unittest='true') and not(@deprecated='true')]">
    <xsl:choose>
      ...
      for (int intIndex=0; intIndex&lt;abytGet_</xsl:text><xsl:value-of select="position()"/><xsl:text>.Length; intIndex++) {
        Assertion.AssertEquals("test get BinaryData", abytGet_</xsl:text><xsl:value-of select="position()"/><xsl:text>[intIndex], dbo.</xsl:text><xsl:value-of select="@name"/><xsl:text>[intIndex]);
      }
</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>      Assertion.AssertEquals("test get </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
        <xsl:call-template name="test-test-value">
          <xsl:with-param name="attribute" select="."/>
        </xsl:call-template>
        <xsl:text>, dbo.</xsl:text><xsl:value-of select="@name"/>
  ...
  <xsl:for-each select="attributes/attribute[position()!=1 and @type!='VSTAMP' and not(@no-unittest='true') and not(@deprecated='true')]">
    <xsl:choose>
      ...
      for (int intIndex=0; intIndex&lt;abytRetrieve_</xsl:text><xsl:value-of select="position()"/><xsl:text>.Length; intIndex++) {
        Assertion.AssertEquals("test retrieve BinaryData", abytRetrieve_</xsl:text><xsl:value-of select="position()"/><xsl:text>[intIndex], dbo2.</xsl:text><xsl:value-of select="@name"/><xsl:text>[intIndex]);
      }
</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>      Assertion.AssertEquals("test retrieve </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
        <xsl:call-template name="test-test-value">
          <xsl:with-param name="attribute" select="."/>
        </xsl:call-template>
...
<xsl:template match="entity" mode="test-constraints">
  ...
  <xsl:if test="$foreign-key-check and $generate-entity/parameters/parameter[@name='generate-removecheck']='true'">
    ...
    public void testCheckRemoveConstraints</xsl:text><xsl:value-of select="position()"/>
      ...
      dbo2.Remove(mcon);
      Assertion.AssertEquals("Constraint must be thrown", true, fRemoveConstraint);
      dbo1.Remove(mcon);
    }
  ...
  <xsl:if test="$foreign-key-cascading-delete">
    ...
    <xsl:text>, mcon);
      ...
      Assertion.AssertEquals("Record must be deleted", null, dbo3);
    }
  ...
  <xsl:for-each select="$generate-entity/custom-create-methods/custom-create-method">
    ...
    <xsl:for-each select="method-attribute">
      ...
      <xsl:if test="$attribute/@type!='VSTAMP'">
        <xsl:text>      Assertion.AssertEquals("test constructor value </xsl:text><xsl:value-of select="@name"/><xsl:text>", </xsl:text>
        <xsl:call-template name="test-test-value">
          <xsl:with-param name="attribute" select="$attribute"/>
        </xsl:call-template>
  ...
  <xsl:param name="pos"/>
    ...
    <xsl:choose>
      <xsl:when test="@returns-multiple='true'">
        <xsl:text>      bool fFound = false;
      </xsl:text><xsl:value-of select="$entity-class"/><xsl:text>[] adbo = </xsl:text><xsl:value-of select="$entity-class"/>
        ...
        <xsl:text>mcon);
      for (int intIndex=0; intIndex&lt;adbo.Length; intIndex++) {
        if (adbo[intIndex].</xsl:text><xsl:value-of select="$ent/attributes/attribute[1]/@name"/><xsl:text>==dbo.</xsl:text>
          <xsl:value-of select="$ent/attributes/attribute[1]/@name"/><xsl:text>) {
          fFound = true;
        }
      }
      Assertion.AssertEquals("test </xsl:text><xsl:value-of select="@name"/><xsl:text> method", true, fFound);
</xsl:text>
      </xsl:when>
      <xsl:otherwise>
      ...
      Assertion.AssertEquals("test </xsl:text><xsl:value-of select="@name"/><xsl:text> method", true, dbo2!=null);
</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
