...
<xsl:template match="patch-entity" mode="modify">
  ...
  <xsl:if test="patch-attribute[@patch-type='DROP']">
    <xsl:text>ALTER TABLE </xsl:text><xsl:value-of select="@entity"/><xsl:text> DROP (</xsl:text>
...
/
  ...
  <xsl:if test="patch-attribute[@patch-type='CREATE']">
    ...
    <xsl:for-each select="patch-attribute[@patch-type='CREATE']">
      <xsl:variable name="cr" select="$db-definition/entities/entity[@name=$entity]/attributes/attribute[@name=current()/@name]"/>
      <xsl:if test="$cr">
        <xsl:if test="position() &gt; 1">
      <xsl:text>,
</xsl:text>
        </xsl:if>
        <xsl:apply-templates select="$cr" mode="create"/>
      </xsl:if>
    </xsl:for-each>
  <xsl:text>)
/
...
/
  ...
  <xsl:if test="patch-attribute[@patch-type='MODIFY']">
    <xsl:text>ALTER TABLE </xsl:text><xsl:value-of select="$entity"/><xsl:text> MODIFY (
</xsl:text>
