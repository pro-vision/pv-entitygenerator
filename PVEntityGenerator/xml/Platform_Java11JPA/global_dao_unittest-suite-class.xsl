...
import org.junit.platform.suite.api.SelectClasses;
  ...
  <xsl:if test="$generate-comments">
    <xsl:text>/**
 * Unit test suite for DAO classes.
...
@SelectClasses( {
  ...
  <xsl:for-each select="$db-definition/entities/entity[key('generate-entity', @name)//parameters/parameter[@name='generate-entity']='true']">
    <xsl:if test="$platform/parameters/parameter[@name='generate-unittest']='true'">
      <xsl:text>  </xsl:text><xsl:value-of select="$platform/parameters/parameter[@name='dao-unittest-package']"/>
      <xsl:text>.</xsl:text><xsl:value-of select="@name"/><xsl:text>DAOTest.class</xsl:text>
